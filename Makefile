...
tests:
	@docker compose exec -it mysql-testing mysql -uroot -proot -e "DROP DATABASE IF EXISTS app_test_test"
	@docker compose exec -it php-fpm php bin/console --env=test doctrine:database:create
	@docker compose exec -it php-fpm php bin/console --env=test doctrine:migrations:migrate
